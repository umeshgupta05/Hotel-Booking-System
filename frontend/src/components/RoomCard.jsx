...
const RoomCard = ({ room, hotelId }) => {
  ...
  const price = room.base_price || room.roomCategory?.basePrice || 0;
  const category = room.category_name || room.roomCategory?.name;
  const capacity = room.capacity || room.roomCategory?.capacity;
  ...
  return (
    <div className="surface-panel hover-lift text-left p-5 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
      <div>
